...
def read_data(flag=0):
    ...
    userdata = {}
    att_static = [set() for x in range(len(gl_attlist)+1)]
    ...
    for k, v in userdata.iteritems():
        if k in conditiondata:
            ...
            for t in conditiondata[k]:
                temp.add(t[2])
                # for static
                att_static[-1].add(t[2])
            hashdata[k] = []
            for i in range(len(gl_attlist)):
                index = gl_attlist[i]
                hashdata[k].append(v[index])
                # for static
                att_static[i].add(v[index])
            hashdata[k].append(list(temp))
    for k, v in hashdata.iteritems():
        data.append(v)
    if __DEBUG:
        print "Attributes Static"
        att_names = [gl_useratt[t] for t in gl_attlist]
        att_names.append('ICD9CODX')
        for i in range(len(att_names)):
            print "%s: %d" % (att_names[i], len(att_static[i]))
    userfile.close()
    conditionfile.close()
    return data
